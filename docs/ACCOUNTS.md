...
ينسخ الأمر معرّف القاعدة. ضعه مكان `REPLACE_WITH_D1_DATABASE_ID` في **`wrangler.jsonc`** و**`wrangler.rooms.jsonc`** معًا (الاثنان يتشاركان القاعدة نفسها، فحساب واحد يخدم الواجهتين). ثم:
...
بدون ربط `DB` تعمل الغرف كما كانت تمامًا، وتردّ مسارات الحسابات `404`.
