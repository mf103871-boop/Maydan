...
- **على الحساب**: مع جلسة صالحة يُرسل الرمز إلى `POST /api/redeem` فيُضاف صف اشتراك مصدره `promo` (100 سنة، بلا تجديد) يراه `/api/me` على كل الأجهزة، ومنها تطبيق iOS. رمز مفعَّل على الجهاز ولم يُربط بعد يُعاد إرساله عند الإقلاع وكل تحديث حتى ينجح (`syncedFor` في المخزن يحفظ الحساب الذي رُبط به). اشتراك مدفوع سارٍ يتقدّم على صف الرمز في `/api/me` كي يبقى مصدره وتجديده و«إدارة الاشتراك» ظاهرين.
- **إضافة رموز أو إلغاؤها**: أضف بصمة إلى `REDEEM_CODE_HASHES` وانشر (تُحسب بـ`node -e "console.log(require('crypto').createHash('sha256').update('CODE'.toUpperCase()).digest('hex'))"`)، أو ضع بصمات إضافية في السرّ `REDEEM_CODE_HASHES` (مفصولة بفواصل): هذه لا تظهر في الحزمة، ولا تُقبل إلا مع جلسة (الواجهة تجرّب الخادم حين لا يعرف الجهاز الرمز)، ولا تعمل دون اتصال. إزالة بصمة من الإعداد تُبطل الرمز على الأجهزة عند التحديث التالي؛ صفوف `promo` على الخادم تُحذف يدويًا من D1 إن لزم.
