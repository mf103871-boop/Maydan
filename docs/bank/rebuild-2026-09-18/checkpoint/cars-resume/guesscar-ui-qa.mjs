...
const OUT=path.resolve(process.argv[2]||path.join(ROOT,'docs/bank/rebuild-2026-09-18/qa-2026-09-25/guesscar/rerun'));
...
const packBytes=await fs.readFile(path.join(ROOT,'src/data/categories/guesscar.json'));
const pack=JSON.parse(packBytes),tiers=[200,400,600,800,1000];
if(pack.qs.length!==40||!pack.qs.every(q=>q.type==='image'&&(!q.effect||q.effect==='none')&&/^guesscar-(200|400|600|800|1000)-90[1-8]$/.test(q.qid)))throw new Error('Wait for the integrated40 car identification questions; old pack must not be tested as new.');
...
options.plugins=[{name:'deterministic-guesscar-batches',setup(builder){
 builder.onLoad({filter:/[/\\]categories[/\\]guesscar\.json$/},()=>({loader:'js',contents:`const pack=${JSON.stringify(pack)}; const n=new URLSearchParams(location.search).get('guesscar-batch'); if(n!==null&&n!=='all'){const i=Number(n);pack.qs=[200,400,600,800,1000].map(p=>pack.qs.filter(q=>q.p===p)[i]);} export default pack;`}));
}}];
const bundle=await build(options),html=await renderHtml(bundle.outputFiles[0].text,{version:pkg.version});
const me={user:{id:'local-guesscar-qa',name:'فحص محلي'},premium:{active:true,until:Date.now()+86400000,source:'test',status:'active',willRenew:false},trials:{},serverTime:Date.now()};
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(pathname==='/sw.js')body=Buffer.from(body.toString().replaceAll('__BUILD_ID__','guesscar-ui-fixture'));
...
async function state(){return page.locator('.m-question-screen').evaluate(el=>{
 const base=el.querySelector('.m-media-img.fx-none'),stage=el.querySelector('.m-media-stage');
 return {text:el.innerText,base:{alt:base.alt,width:base.naturalWidth,height:base.naturalHeight,filter:getComputedStyle(base).filter},count:el.querySelectorAll('.m-media-img').length,canEnlarge:stage.classList.contains('can-enlarge'),stageRole:stage.getAttribute('role'),attributes:[...el.querySelectorAll('img,[title],[aria-label]')].map(x=>({alt:x.getAttribute('alt'),title:x.getAttribute('title'),aria:x.getAttribute('aria-label'),src:x.tagName==='IMG'?x.getAttribute('src'):null}))};
 });}
...
try{
 ...
 for(let batch=0;batch<8;batch++){
  ...
  const route=`${BASE}/?guesscar-batch=${batch}`;
  await page.goto(`${route}#/auth?code=local-guesscar-qa`,{waitUntil:'domcontentloaded'});
  ...
  for(const tier of tiers){
   ...
   await page.waitForFunction(()=>[...document.querySelectorAll('.m-media-img')].length===1&&[...document.querySelectorAll('.m-media-img')].every(img=>img.complete&&img.naturalWidth>0));
   await settle();const before=await state();
   check(`${q.qid}: single clear car image before reveal`,before.base.filter==='none'&&before.count===1&&before.base.width>0,before);
   ...
   check(`${q.qid}: enlargement available before reveal`,before.canEnlarge&&before.stageRole==='button');
   await page.locator('.m-media-stage').click();await page.locator('.m-lightbox').waitFor();check(`${q.qid}: enlarged question retains generic alt`,await page.locator('.m-lightbox img').getAttribute('alt')==='صورة السؤال');await page.getByRole('button',{name:'إغلاق الصورة',exact:true}).click();
   const sizing=[];
   ...
   const beforeFile=path.join(OUT,'stages',`${q.qid}-before.png`),afterFile=path.join(OUT,'stages',`${q.qid}-after.png`);
   ...
   check(`${q.qid}: clear car image remains after reveal`,after.count===1&&after.base.filter==='none'&&after.canEnlarge,after);
   check(`${q.qid}: answer matches after reveal`,after.base.alt===q.a&&(await page.locator('.m-answer strong').innerText())===q.a);
   ...
   const beforePixels=await imagePixels(beforeFile),afterPixels=await imagePixels(afterFile);
   check(`${q.qid}: before and after image dimensions match`,beforePixels.width===afterPixels.width&&beforePixels.height===afterPixels.height,{beforePixels,afterPixels});
   check(`${q.qid}: same neutral source remains after reveal`,before.attributes.find(x=>x.src)?.src===after.attributes.find(x=>x.src)?.src);
  ...
  if(batch===7){
   ...
   await page.goto(`${BASE}/?guesscar-batch=all&credits=1#/about`,{waitUntil:'domcontentloaded'});await page.locator('.about-list').waitFor();
...
}catch(error){check('guesscar gameplay flow',false,error.stack||error.message);await page?.screenshot({path:path.join(OUT,'failure.png'),fullPage:true,animations:'disabled'}).catch(()=>{});}
finally{
...
// Diagnostic contact sheets contain only actual gameplay screenshots, question and
// answer-shown side by side. Labels are neutral IDs; originals remain available.
for(const tier of tiers){
 ...
 for(let i=0;i<group.length;i++){
  ...
  const label=Buffer.from(`<svg width="528" height="28"><rect width="528" height="28" fill="#ffffff"/><text x="12" y="20" font-size="16" font-family="Arial">${q.qid} | IMAGE BEFORE / ANSWER SHOWN</text></svg>`);
