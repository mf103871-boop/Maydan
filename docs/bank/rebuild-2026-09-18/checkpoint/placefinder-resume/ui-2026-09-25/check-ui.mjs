...
const ROOT=path.resolve(process.env.MAYDAN_REPO||path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../../../../../..'));
const OUT=path.resolve(process.argv[2]||process.env.MAYDAN_UI_OUT||path.join(process.cwd(),'placefinder-ui-qa-output'));
...
const packBytes=await fs.readFile(path.join(ROOT,'src/data/categories/placefinder.json'));
const pack=JSON.parse(packBytes),tiers=[200,400,600,800,1000];
if(pack.qs.length!==40||!pack.qs.every(q=>q.type==='image'&&q.effect==='none'&&/^placefinder-(200|400|600|800|1000)-90[1-8]$/.test(q.qid)))throw new Error('Wait for the integrated40 landmark questions; old pack must not be tested as new.');
...
options.plugins=[{name:'deterministic-placefinder-batches',setup(builder){
 builder.onLoad({filter:/[/\\]categories[/\\]placefinder\.json$/},()=>({loader:'js',contents:`const pack=${JSON.stringify(pack)}; const n=new URLSearchParams(location.search).get('placefinder-batch'); if(n!==null&&n!=='all'){const i=Number(n);pack.qs=[200,400,600,800,1000].map(p=>pack.qs.filter(q=>q.p===p)[i]);} export default pack;`}));
}}];
const bundle=await build(options),html=await renderHtml(bundle.outputFiles[0].text,{version:pkg.version});
const me={user:{id:'local-placefinder-qa',name:'فحص محلي'},premium:{active:true,until:Date.now()+86400000,source:'test',status:'active',willRenew:false},trials:{},serverTime:Date.now()};
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(pathname==='/sw.js')body=Buffer.from(body.toString().replaceAll('__BUILD_ID__','placefinder-ui-fixture'));
...
async function state(){return page.locator('.m-question-screen').evaluate(el=>{
 const base=el.querySelector('.m-media-img'),stage=el.querySelector('.m-media-stage');
 return {text:el.innerText,base:{alt:base.alt,width:base.naturalWidth,height:base.naturalHeight,opacity:getComputedStyle(base).opacity,filter:getComputedStyle(base).filter,hidden:base.classList.contains('is-hidden-src')},canEnlarge:stage.classList.contains('can-enlarge'),attributes:[...el.querySelectorAll('img,[title],[aria-label]')].map(x=>({alt:x.getAttribute('alt'),title:x.getAttribute('title'),aria:x.getAttribute('aria-label'),src:x.tagName==='IMG'?x.getAttribute('src'):null}))};
 });}
...
try{
 ...
 for(let batch=0;batch<8;batch++){
  ...
  const route=`${BASE}/?placefinder-batch=${batch}`;
  await page.goto(`${route}#/auth?code=local-placefinder-qa`,{waitUntil:'domcontentloaded'});
  ...
  for(const tier of tiers){
   ...
   check(`${q.qid}: complete original visible before reveal`,before.base.opacity==='1'&&!before.base.hidden&&before.base.width>0,before.base);
   check(`${q.qid}: no distortion filter or answer leak`,before.base.filter==='none'&&before.base.alt==='صورة السؤال'&&leakAnswers(q,before).length===0,{filter:before.base.filter,leaks:leakAnswers(q,before)});
   check(`${q.qid}: original can enlarge before reveal`,before.canEnlarge);
   await page.locator('.m-media-stage.can-enlarge').click();await page.locator('.m-lightbox').waitFor();
   check(`${q.qid}: enlarged question retains generic alt`,await page.locator('.m-lightbox img').getAttribute('alt')==='صورة السؤال');
   ...
   check(`${q.qid}: complete original remains visible after reveal`,after.base.opacity==='1'&&!after.base.hidden&&after.canEnlarge,after.base);
  ...
  if(batch===7){
   ...
   await page.goto(`${BASE}/?placefinder-batch=all&credits=1#/about`,{waitUntil:'domcontentloaded'});await page.locator('.about-list').waitFor();
...
}catch(error){check('placefinder gameplay flow',false,error.stack||error.message);await page?.screenshot({path:path.join(OUT,'failure.png'),fullPage:true,animations:'disabled'}).catch(()=>{});}
finally{
...
for(const tier of tiers){
 ...
 for(let i=0;i<group.length;i++){
  ...
  const label=Buffer.from(`<svg width="528" height="28"><rect width="528" height="28" fill="#ffffff"/><text x="12" y="20" font-size="16" font-family="Arial">${q.qid} | BEFORE / ANSWER REVEALED</text></svg>`);
