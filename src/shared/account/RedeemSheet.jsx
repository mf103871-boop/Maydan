...
export function RedeemForm({ onDone, onCancel }) {
  ...
  const submit = async (event) => {
    ...
    setBusy(true);
    setError('');
    const ok = await account.redeem(value);
    setBusy(false);
    if (ok) { setCode(''); if (onDone) onDone(); }
    else setError(accountErrorText('REDEEM_INVALID'));
  };
  return (
    <form className="account-redeem" onSubmit={submit} aria-label="تفعيل رمز الهدية">
      <p className="card-muted">أدخل الرمز الذي حصلت عليه لتفعيل {PLUS_NAME} على هذا الجهاز. إن سجّلت الدخول يُربط بحسابك ليعمل على كل أجهزتك.</p>
