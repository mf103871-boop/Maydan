...
function Invite({ code }) {
  ...
  return <Card className="online-invite">
    ...
    {qr && <img width="136" height="136" src={qr} alt={`رمز QR للدخول إلى الغرفة ${code}`} />}
...
function Match({ state, disabled, isHost, act, clockOffset }) {
  ...
  return <>
    <div className="row-between"><b>الجولة {arabic(state.round)} من {arabic(state.rounds)}</b>{state.phase === 'vote' && <span className={`online-timer ${seconds <= 5 ? 'urgent' : ''}`} aria-label={`متبقي ${seconds} ثانية`}><bdi>{arabic(seconds)}</bdi> ث</span>}</div>
    <Card className="online-question"><GameArtwork game="meenfina" /><h1>{state.question?.text}</h1></Card>
    {state.phase === 'vote' ? <Card className="stack">
      {state.myVote.submitted ? <div className="online-voted" role="status"><strong>وصل تصويتك ✓</strong><p>اختيارك: {state.myVote.targetId === null ? 'لا أحد' : players.find((p) => p.id === state.myVote.targetId)?.name}. بانتظار الباقي…</p></div> : <>
        ...
        <div className="online-vote-grid" role="group" aria-label="اختر لاعبًا">{players.map((p) => <button className={`online-vote-choice ${choice === p.id ? 'selected' : ''}`} type="button" key={p.id} disabled={disabled || seconds === 0} aria-pressed={choice === p.id} onClick={() => setChoice(p.id)}><Avatar index={p.avatar} /><b>{p.name}</b>{p.left && <small>غادر الجولة</small>}</button>)}</div>
      ...
      <ul className="online-players">{players.map((p) => <li key={p.id}><Avatar index={p.avatar} /><b className="grow">{p.name}</b><span>{arabic(state.counts[p.id] || 0)} أصوات</span>{state.winners.includes(p.id) && <b className="online-point">+١</b>}</li>)}</ul>
...
function ConnectedRoom({ code, session, stored, onLeft }) {
  const { toast, sound, haptics } = usePlatform();
  ...
  useEffect(() => { if (phase === 'result') { sound.play('pop'); haptics.vibrate('success'); } }, [phase]);
